...
import { getDashboardPath, getUserRole, createUserProfile } from '../utils/auth'
...
function getFirebaseAuthMessage(error) {
  ...
  if (code === 'permission-denied') {
    return 'Unable to access your profile. Please check your account and try again.'
  }

  if (code === 'network-error' || error?.message?.includes('Network')) {
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(firebaseAuth, (user) => {
      ...
      console.log(`[Login] Auth state changed, user: ${user.uid}`)
      getUserRole(user.uid)
        .then((storedRole) => {
          if (storedRole) {
            console.log(`[Login] Auto-redirect to dashboard with role: ${storedRole}`)
            navigate(getDashboardPath(storedRole), { replace: true })
          } else {
            console.warn('[Login] User logged in but role not found')
          }
        })
        .catch((err) => {
  ...
  async function handleSignIn(event) {
    ...
    try {
      ...
      console.log(`[Login] Auth successful for user: ${credential.user.uid}`)
      
      // Try to get user profile, but handle permission errors gracefully
      try {
        ...
        if (userSnap.exists()) {
          ...
          if (storedRole) {
            console.log(`[Login] User profile found with role: ${storedRole}`)
            await updateDoc(userRef, { lastLogin: serverTimestamp() }).catch(err => {
              console.warn('[Login] Could not update lastLogin:', err.message)
            })
        ...
        console.error('[Login] Error reading profile:', profileError.code, profileError.message)
        // Profile read failed, try to auto-create profile
      }

      // If profile doesn't exist or can't be read, auto-create it as patient
      ...
      try {
        ...
        navigate(getDashboardPath('patient'), { replace: true, state: { from: location } })
      } catch (createError) {
      ...
      console.error('[Login] Sign in error:', signInError.code, signInError.message)
  ...
  async function handleSignUp(event) {
    ...
    try {
      ...
      console.log(`[Login] User account created: ${credential.user.uid}`)
      
      if (name.trim()) {
        await updateProfile(credential.user, { displayName: name.trim() })
        console.log('[Login] Updated display name')
      }
      ...
      console.log(`[Login] User profile created, navigating to dashboard...`)
      navigate(getDashboardPath(role), { replace: true, state: { from: location } })
    } catch (signUpError) {
      console.error('[Login] Sign up error:', signUpError.code, signUpError.message)
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      console.log(`[Login] Google auth successful: ${result.user.uid}`)
      ...
      if (existingRole) {
        console.log(`[Login] Existing user found with role: ${existingRole}`)
        await updateDoc(userRef, { lastLogin: serverTimestamp() })
        navigate(getDashboardPath(existingRole), { replace: true, state: { from: location } })
      } else {
        console.log('[Login] New Google user, prompting for role selection')
        setPendingGoogleUser(result.user)
      }
    } catch (googleError) {
      console.error('[Login] Google sign in error:', googleError.code, googleError.message)
  ...
  function handleCompleteGoogleRole() {
    ...
    setLoading(true)
    setError('')
    console.log(`[Login] Completing Google sign up with role: ${googleRole}`)
    
    createUserProfile(pendingGoogleUser.uid, {
      name: pendingGoogleUser.displayName || '',
      email: pendingGoogleUser.email || '',
      role: googleRole,
    })
      .then(() => {
        console.log('[Login] Google profile created successfully')
        setPendingGoogleUser(null)
        navigate(getDashboardPath(googleRole), { replace: true, state: { from: location } })
      })
      .catch((error) => {
        console.error('[Login] Error completing Google sign up:', error)
